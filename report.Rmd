...
str(breweriesbystate)
#head(breweriesbystate)
# validating the count for state of Texas
#test <- grep("TX",breweriesOrderedByState$State,value = TRUE)
#length(test)
#[1] 28
#write.csv(breweriesbystate,file ="data/breweriesbystate.csv", row.names = FALSE, quote=FALSE)
print(breweriesbystate)
```
Observed that in the breweries dataset, the state of colorado has the highest number of breweries followed by california. Also, the least number of breweries are present in four states, District of Columbia, North Dakota, South Dakota and West Virginia
...
#head(Beers)
#tail(Beers)
names(Breweries) # dim 558 4
names(Beers) # dim 2410 7
#rename columns
...
```
State Colorado (CO) has the maximum alcoholic (ABV) beer. State Oregon (OR) has the most bitter (IBU) beer.
