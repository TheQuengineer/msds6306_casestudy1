...
```{r}
library(ggplot2)
bitterness <- aggregate(breweriesandbeers$IBU, by=data.frame(breweriesandbeers$State), median, na.rm=TRUE)
bitterness
alcohol <- aggregate(breweriesandbeers$ABV, by=data.frame(breweriesandbeers$State), median, na.rm=TRUE)
alcohol
ggplot(bitterness, aes(x=bitterness$breweriesandbeers.State, y=bitterness$x))+geom_bar(stat = "identity")+labs(title="IBU comparison by state", x="State", y="IBU value")
ggplot(alcohol, aes(x=alcohol$breweriesandbeers.State, y=alcohol$x))+geom_bar(stat = "identity")+labs(title="ABV comparison by state", x="State", y="ABV value")
```
